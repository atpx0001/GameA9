...
public class IntTweenProperty : AbstractTweenProperty, IGenericProperty
{
    ...
    public IntTweenProperty(Func<int> getter, Action<int> setter, int endValue, bool isRelative = false)
        : base(isRelative) {
        this._setter = setter;
        this._getter = getter;
        _originalEndValue = endValue;
	...
	{
		// retrieve the getter
        var getter = _getter != null ? _getter : GoTweenUtils.getterForProperty<Func<int>>(_ownerTween.target, propertyName);
