...
public class GoDummyPathEditor : Editor
{
	...
	public override void OnInspectorGUI()
	{
		...
		EditorGUILayout.HelpBox( "While dragging a node, hold down Ctrl and slowly move the cursor to snap to a nearby point\n\n" +
                       ...
                       "Hold down shift while adding result node to insert the new node before (left mouse button) or behind (right mouse button) the nearest node\n\n" +
		...
		if( Event.current.type == EventType.mouseDown )
		{
			...
			if( Event.current.clickCount > 1 )
			{
                ...
                if(Event.current.shift) {
                    int index = getNearestNodeForMousePosition(Event.current.mousePosition, float.MaxValue);
                    if(Event.current.button == 0) {
                        insertNodeAtIndex(translatedPoint, index);
                    } else {
                        insertNodeAtIndex(translatedPoint, index + 1);
                    }
    ...
    private int getNearestNodeForMousePosition(Vector3 mousePos, float minDistance = 10) {
        var bestDistance = float.MaxValue;
        var index = -1;
